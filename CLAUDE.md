...
The "Interactive Demos" nav category (`content/projects/interactive-demos/`) contains live, LLM-powered tools. Both use Cloudflare Worker API routes in `src/worker.js` that proxy to Anthropic's API. The API key is stored as a Cloudflare secret (`ANTHROPIC_API_KEY`), never in source code. Local dev uses `.dev.vars` (gitignored).

- **`style-editor`** (`content/projects/interactive-demos/style-editor.xml`) — AI Style Guide Editor. Route: `/api/style-check`.
- **`chatbot-demo`** (`content/projects/interactive-demos/chatbot-demo.xml`) — GskiFlix Support Chatbot. Route: `/api/chat`.
